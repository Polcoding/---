...
다음 추천 작업은 새 외부 도구 추가, 새 output 생성, 한컴 수동 확인이 아니라, 사용자가 저장소 밖 비식별 HWPX 작업 복사본을 준비했다고 명시했을 때 구조 선확인을 어떤 순서와 기준으로 수행할지 문서상으로만 더 좁히는 것입니다.
...
4. 비식별 HWPX 후보가 준비됐다고 명시된 경우에도 `--template ... --no-output` 구조 선확인을 먼저 수행하고, 실제 HWPX preview, 새 output 생성, 실제 원본 또는 개인정보 가능성이 보이면 즉시 사용자 확인 요청
