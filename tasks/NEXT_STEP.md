...
다음 추천 작업은 새 외부 도구 추가가 아니라, 내부 Python 구조 분석 결과와 placeholder-free profile-aware 샘플 주입 PoC를 `docs/150_manual_preview_resume_gate.md`와 template intake 체크리스트의 선확인 항목으로 더 직접 연결하는 것입니다.

현재 샘플 HWPX 1/2의 profile-aware output은 사용자 수동 확인까지 완료되었으므로, 다음 추천 작업은 새 렌더러 기능 추가가 아니라 샘플 profile을 유지하면서 다른 비식별 주제 입력에도 같은 구조와 bullet 정책이 유지되는지 회귀 검증하는 것입니다.

비식별 주제 4개에 대한 샘플 HWPX 1/2 회귀검증은 `renderers/hwpx_renderer/render_autofill_regression_poc.py`로 실행 가능하며, 총 8건이 ignored output 아래에 생성됩니다. 일반 사용은 `python .\renderers\hwpx_renderer\render_autofill_batch_poc.py "[비식별 주제]"`처럼 주제만 넣는 방식으로 단순화했습니다.

사용자가 직접 입력할 최소 항목과 `[확인 필요]`로 남길 항목은 batch 실행 옵션으로 1차 분리했습니다. 현재 `--purpose`, `--target`, `--period`, `--cooperation`, `--background`, `--scope`를 선택 입력으로 받을 수 있고, 제공되지 않은 값은 `[확인 필요]`로 유지합니다.

다음 추천 작업은 선택 입력을 넣은 output을 한컴에서 한 번 확인하고, 문제가 없으면 샘플 HWPX 자동작성 PoC 1차 완료 기준을 정리하는 것입니다. 실제 기관 양식, 실제 원문, 표 내부 실제 데이터 자동화는 계속 제외합니다.
