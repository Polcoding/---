...
복사본 없음 보류 기록은 `docs/134_actual_hwpx_manual_rehearsal_no_copy_hold.md`에 정리했습니다.

HWPX 보고서와 Excel/한셀 표 데이터 역할 분리 범위는 `docs/135_hwp_report_and_hancell_table_data_scope.md`에 정리했습니다.

## 목표

복사본 없이 가능한 다음 내부 작업으로, HWPX 보고서 4종 사용자 입력 템플릿에 표 데이터 후보와 확인 필요 항목을 어떻게 표시할지 검토합니다.
...
- `checklists/hwp_report_and_hancell_table_data_scope_checklist.md`
- `checklists/actual_hwpx_manual_rehearsal_no_copy_hold_checklist.md`
- `README.md`
- `AGENTS.md`

## 확인 항목

1. HWPX 보고서 4종 입력 템플릿에 표 데이터 후보를 실제값 없이 표시할 필요가 있는지
2. 표 데이터 후보를 `표 데이터 별도 확인 필요`, `[확인 필요]`, `null` 중 어떤 방식으로 표현할지
3. `missing_fields`에 수량, 금액, 대상 목록, 일정 같은 표 데이터 확인 필요 항목을 어떻게 보여줄지
4. HWPX 본문 placeholder와 Excel/한셀 후보 데이터를 혼동하지 않는지
5. 실제 표 데이터, 물품명, 수량, 금액, 대상 목록을 저장소에 기록하지 않는지
6. 실제 Excel/한셀 파일 생성이나 자동 연동 구현으로 넘어가지 않는지
...
- HWPX 보고서 본문과 Excel/한셀 표 데이터 후보가 구분됨
- 사용자 입력 템플릿에서 표 데이터 확인 필요 항목을 실제값 없이 표시할 수 있음
...
다음 단계는 `docs/136` 형태로 HWPX 보고서 4종 사용자 입력 템플릿의 표 데이터 후보 표시 기준을 정리하는 것입니다.
