...
테스트 계정과 테스트 데이터 기준을 문서 기준으로 구체화합니다.
...
1. 실제 업무 계정과 테스트 계정의 경계를 문서 기준으로 구분
2. placeholder 기반 테스트 데이터와 실제 업무 데이터의 경계를 정리
3. 외부 서비스로 전송 가능한 테스트 입력 범위를 보수적으로 제한
4. 테스트 계정, 비용, 장애, 권한 문제가 발생했을 때 보류 상태를 정리
...
- 테스트 계정과 테스트 데이터 기준 구체화 필요 여부 판단
- 실제 업무 계정과 테스트 계정의 경계 구분
- placeholder 기반 테스트 데이터 기준 정리
...
현재 추천은 코드 변경 없이 테스트 계정과 테스트 데이터 기준을 문서 기준으로 구체화하는 것입니다. 실제 API, Make.com, Email 연동 구현은 계속 보류합니다.
