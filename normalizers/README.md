...
`hwpx_template_structure_analyzer_poc.py`는 HWPX 본문을 추출하지 않고 section, paragraph, table, placeholder 구조만 확인합니다.

같은 문단 안에서 서식 때문에 여러 text node로 나뉜 placeholder도 구조 확인 대상으로 감지합니다.

비식별 작업 복사본 후보를 저장소 밖에서 선확인할 때는 출력 파일 없이 콘솔만 볼 수 있습니다.

```powershell
python .\normalizers\hwpx_template_structure_analyzer_poc.py --template "[비식별 HWPX 후보 경로]" --no-output
```

이 명령은 본문 텍스트를 저장하지 않고 구조 메타정보만 출력합니다.
...
- HWPX 구조 분석 결과에도 실제 본문 텍스트를 저장하지 않습니다.
