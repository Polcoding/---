...
| [x] | 표 관련 gap은 표 내부 데이터가 아니라 표 틀, 위치, 폭, 줄바꿈, 겹침으로 제한했는가 |
...
| [x] | 실제 표 데이터, 물품명, 수량, 금액, 대상 목록을 기록하지 않도록 했는가 |
...
| [x] | 비식별 작업 복사본 준비가 명시된 경우에만 저장소 밖 한컴 preview 결과의 `table_scope: frame_only` 포함 실제값 없는 gap log 기록으로 재개하는가 |
