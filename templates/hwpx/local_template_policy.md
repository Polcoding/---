...
현재 단계에서는 실제 기관 HWPX 원본이나 실제 업무용 HWPX output을 Git 저장소에 보관하지 않습니다.

## 로컬 보관 원칙

- 실제 기관 HWPX 원본과 실제 업무용 HWPX output은 기본적으로 Git에 커밋하지 않습니다.
...
- 수동 preview와 gap log 기록은 `docs/150_manual_preview_resume_gate.md` 조건이 충족된 뒤에만 진행합니다.
...
- 비식별 HWPX 작업 복사본이 없으면 실제 양식 수동 preview를 진행하지 않고 보류합니다.
