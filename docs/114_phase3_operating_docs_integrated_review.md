...
외부 연동 필요성과 보류 기준은 `docs/115`에, 로그와 감사 추적 기준은 `docs/116`에, 테스트 계정과 테스트 데이터 기준은 `docs/117`에, 실제 원문 차단과 비식별 입력 확인 절차는 `docs/118`에 후속 반영했습니다.

다음 단계에서는 실제 구현 없이 사용자 preview와 사람 승인 지점을 외부 전송 전 절차로 통합 점검합니다.
