...
현재 다음 단계는 실제 연동 구현이 아니라 저장소 밖 실제 양식 후보 수동 절차와 보류 조건을 문서로 정리하는 것입니다.
