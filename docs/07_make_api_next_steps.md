...
현재 다음 단계는 실제 연동 구현이 아니라 실제 양식 수동 리허설 사용자 확인 패킷과 gap log 빈 양식을 문서로 정리하는 것입니다.
