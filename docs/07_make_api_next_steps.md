...
현재 다음 단계는 실제 연동 구현이 아니라 저장소 밖 한컴 preview 결과를 실제값 없는 gap log로 기록하는 것입니다.
