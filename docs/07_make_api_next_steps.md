...
현재 다음 단계는 실제 연동 구현이 아니라 style profile의 `[확인 필요]` 값 유지 기준과 수집 체크리스트를 문서로 정리하는 것입니다.
