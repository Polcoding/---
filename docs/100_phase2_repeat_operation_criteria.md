...
이 기준은 `docs/112_phase3_external_hwpx_and_manual_preview_criteria.md`의 Phase 3 preview 기준과 연결됩니다. 새로운 HWPX 수동 preview 재개 여부는 `docs/150_manual_preview_resume_gate.md` 조건을 먼저 확인합니다.
...
| 12 | Phase 4 문서 템플릿 안정화 진입 여부 | 후속 문서 기준 판단 완료, 현재는 검증 상태 유지 |
...
1. 실제 연동 구현 없이 현재 문서 기반 검증 상태 유지
...
5. 수동 preview는 `docs/150_manual_preview_resume_gate.md` 조건 충족 후에만 재개
