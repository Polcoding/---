...
- `renderer_hints.table_template`은 서식 또는 표시 후보 힌트이며 실제 표 데이터가 아닙니다.
- 보고서 샘플 JSON 4종에는 `table_data_candidate` 같은 새 필드를 직접 추가하지 않습니다.
...
- 이 샘플에 실제 표 데이터, 물품명, 수량, 금액, 대상 목록을 추가하지 않습니다.

## 보고서 샘플 표 데이터 후보 기준

보고서 샘플 JSON 4종의 표 데이터 후보 직접 반영 여부는 `docs/137_report_sample_json_table_data_candidate_review.md` 기준을 따릅니다.

현재 단계에서는 샘플 JSON을 HWPX payload 확장 대상으로 보지 않고, placeholder 기반 renderer 검증 샘플로 유지합니다.
