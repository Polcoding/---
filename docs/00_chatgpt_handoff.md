...
다음 단계는 API/Make.com 연동이 아니라 `placeholder_confirmed_values` read-only 판정 helper를 최소 범위로 구현하고, 기존 fixture 회귀 테스트를 통과시키는 것입니다.

이 helper는 `missing_fields` 생성 결과와 routing 결과를 즉시 바꾸지 않아야 합니다.
