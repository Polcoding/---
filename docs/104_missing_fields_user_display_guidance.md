...
표 데이터 후보는 현재 고정 `missing_fields` 생성 규칙을 바꾸지 않습니다.

다만 사용자에게 검토용 목록으로 보여줄 때는 다음 표시명을 후보로 사용할 수 있습니다.

| 내부 후보 | 한글 표시명 | 권장 임시 표시 | 주의 |
|---|---|---|---|
| `table_data` | 표 데이터 | `[표 데이터 별도 확인 필요]` | 표가 필요한지 확인하는 표시이며 실제 표 데이터가 아님 |
| `quantity` | 수량 | `[확인 필요]` | 실제 수량을 임의 생성하지 않음 |
| `amount` | 금액 | `[확인 필요]` | 실제 금액이나 예산액을 입력하지 않음 |
| `target_list` | 대상 목록 | `[확인 필요]` | 실제 기관, 부서, 개인, 구역 목록을 기록하지 않음 |
| `schedule_table` | 일정표 | `[확인 필요]` | 실제 날짜와 담당자를 임의 생성하지 않음 |

위 후보는 사용자 표시 기준입니다. 현재 코드의 `make_missing_fields()` 결과, routing 결과, HWPX payload 구조를 변경하지 않습니다.

권장 표시 예:

| 순번 | 확인 항목 | 확인 사유 | 관련 위치 | 임시 표시 | 사용자 조치 |
|---|---|---|---|---|---|
| 1 | 표 데이터 | 표 데이터 필요 여부 확인 필요 | `project_plan.table_data_candidate` | `[표 데이터 별도 확인 필요]` | 내부 검토 후 표 후보 유지 또는 제외 |

이 예시는 구조만 보여줍니다. 실제 수량, 금액, 일정, 대상 목록은 넣지 않습니다.
...
이 섹션은 역할 구분용입니다. 실제 일정, 예산, 실적, 수량, 금액, 대상 목록을 기록하지 않습니다.
...
- 표 데이터 후보를 이유로 실제 수량, 금액, 대상 목록, 일정표를 작성하려는 경우
...
- `[사용자 확인 필요]` 항목을 Codex가 임의 작성하려는 경우
- `[Codex 처리 가능]` 항목을 실제값 생성 권한으로 오인하는 경우
- `[보류]` 항목을 현재 단계 실행 대상으로 처리하려는 경우
...
표 데이터 후보는 `[표 데이터 별도 확인 필요]`로 표시할 수 있지만, 실제 표 데이터나 자동 연동 구현을 의미하지 않습니다.

이 기준은 Phase 2 반복 운영 로그 템플릿과 수동 점검표에 반영되어 있습니다.

현재 다음 단계는 실제 코드 변경이 아니라, 사용자 입력 안내 문서와 체크리스트에서 `[사용자 확인 필요]`, `[Codex 처리 가능]`, `[보류]` 표시가 실제값 입력 지시처럼 오해되지 않는지 유지 점검하는 것입니다.
