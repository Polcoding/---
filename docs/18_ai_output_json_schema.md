...
HWPX 보고서 자동화에서는 one_page_report를 최우선 후보로 두고, project_plan과 result_report를 핵심 대상 문서로 관리합니다. result_report는 가능한 경우 기존 project_plan의 추진개요, 세부계획, 일정, 예산, 기대효과 항목과 대응되도록 구성합니다.

Email 계열 document_type은 HWPX 보고서 자동화의 보조 기능이며, 협조 요청 메일은 공문체가 아니라 정중한 실무 메일체를 사용합니다.
...
표 데이터 후보가 필요한 경우에도 실제 표 행, 실제 수량, 실제 금액, 실제 대상 목록을 만들지 않습니다. 사용자 확인용 항목으로만 분리하고, 본문에는 `[표 데이터 별도 확인 필요]` 또는 `[확인 필요]`를 유지합니다.
...
[
  ...
  },
  {
    ...
    "reason": "표 데이터 필요 여부와 범위 확인 필요",
    "required_for": "project_plan.table_data_candidate",
    "suggested_placeholder": "[표 데이터 별도 확인 필요]"
  }
]
```
...
원페이지 보고서는 HWPX 보고서 템플릿 자동화의 최우선 대상입니다.
...
원페이지 보고서는 개조식과 보고서체를 우선하며, 실제 담당자명, 문서번호, 예산액, 일정, 수량은 임의 생성하지 않습니다.

```json
{
  "title": "[보고 제목 확인 필요]",
  "report_overview": "[보고 개요 확인 필요]",
  "background": "[추진 배경 확인 필요]",
  "main_contents": ["[주요 내용 확인 필요]"],
  "review_opinion": "[검토 의견 확인 필요]",
  "issues_or_considerations": ["[고려사항 확인 필요]"],
  "future_plan": "[향후계획 확인 필요]",
  "action_items": ["[조치 필요사항 확인 필요]"],
  "attachments": [],
  "checklist": ["개조식 여부 확인", "확인 필요 항목 분리", "개인정보 제외 여부 확인"]
}
```
...
linked_project_plan은 실제 계획서 원문이나 실제 파일명을 저장하지 않고, 비식별 placeholder로만 연결합니다. plan_result_mapping은 계획서의 주요 항목과 결과보고서 항목을 대응시키기 위한 구조이며, 실제 수치와 결과값은 사용자가 제공하지 않으면 [확인 필요]로 표시합니다.
...
```json
{
  ...
  "linked_project_plan": {
    "project_plan_id": "[계획서 ID placeholder]",
    "project_plan_title": "[기존 추진계획서 제목 확인 필요]",
    "link_basis": "[사용자 제공 정보 기준]"
  },
  "plan_result_mapping": [
    {
      "plan_item": "[계획서 주요 항목]",
      "result_item": "[결과보고서 대응 항목]",
      "status": "[확인 필요]",
      "notes": "[계획 대비 결과 확인 필요]"
    }
  ],
  "results": ["[추진결과 확인 필요]"],
...
HWPX 보고서의 표 데이터 후보는 현재 단계에서 실제 Excel/한셀 연동이나 HWPX payload 확장을 의미하지 않습니다. `table_template`은 서식 또는 표시 후보 힌트로만 사용하고, 실제 표 내부 데이터는 사람이 로컬에서 검토합니다.
