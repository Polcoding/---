...
output 요약 파일은 로컬 검증 산출물이며 Git 제외 상태로 유지합니다.
...
작은 문서 하나가 추가될 때마다 push를 권하지 않고, 결과물 지도와 주요 진입점 정합성까지 함께 확인한 뒤 push 여부를 판단합니다.

## 다음 추천

다음에는 이 결과물 지도가 `README.md`, `CURRENT_STATUS.md`, `AGENTS.md`, `tasks/NEXT_STEP.md`와 서로 모순 없이 연결되는지 점검합니다.
