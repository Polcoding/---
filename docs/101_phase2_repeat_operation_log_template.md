...
한컴 수동 preview와 gap log 기록은 `docs/150_manual_preview_resume_gate.md` 조건이 충족된 경우에만 작성합니다. 조건이 충족되지 않으면 해당 항목은 `[보류]`로 남깁니다.
...
이 섹션은 `docs/150_manual_preview_resume_gate.md` 조건 충족 후에만 사용합니다. 조건이 충족되지 않았으면 파일을 열지 않고 결과를 `[보류]`로 기록합니다.
...
| 확인 | 항목 | 결과 |
|---|---|---|
| [ ] | HWPX preview 상태 (`docs/150_manual_preview_resume_gate.md` 기준) | [통과 / 보류 / 실패 / 사용자 확인 필요] |
...
현재 다음 단계는 실제 연동 구현이 아니라, 사용자 입력 안내 문서와 체크리스트에서 `[사용자 확인 필요]`, `[Codex 처리 가능]`, `[보류]` 표시와 `docs/150_manual_preview_resume_gate.md` 기준이 눈에 잘 보이는지 유지 점검하는 것입니다.
