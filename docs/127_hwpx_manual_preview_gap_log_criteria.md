...
- 표가 포함된 문서는 표 내부 데이터가 아니라 표 틀, 위치, 폭, 줄바꿈, 겹침만 기록합니다.
...
| `표 폭` | 표 또는 표 역할 영역의 폭이 좁거나 넓음. 현재 단계에서는 표 내부 데이터가 아니라 표 틀만 확인 | `일정 영역 폭 확인 필요` |
...
현재 단계에서 HWPX 표는 표 내부 데이터 자동화 대상이 아닙니다.
...
표 데이터 자동화는 향후 Excel/한셀 연동 후보로 분리합니다.
