...
- 비식별 HWPX 작업 복사본이 없으면 실제 양식 수동 preview 결과를 임의로 기록하지 않음
...
| 복사본 없음 보류 | `docs/134` | 비식별 HWPX 작업 복사본이 없으면 실제 preview 결과 기록 보류 |
| 표 데이터 역할 분리 | `docs/135` | HWPX 본문과 Excel/한셀 표 데이터 후보 분리 |
...
사용자의 저장소 밖 준비 확인 기록은 `docs/132_actual_hwpx_manual_rehearsal_readiness_confirmation.md`에 정리했습니다.

HWPX 일원화 유지와 표 데이터 Excel/한셀 연동 후보 분리 결정은 `docs/133_hwpx_only_table_frame_decision.md`에 정리했습니다.

비식별 HWPX 작업 복사본 없음에 따른 실제 양식 수동 리허설 보류 기록은 `docs/134_actual_hwpx_manual_rehearsal_no_copy_hold.md`에 정리했습니다.
...
현재 기준에서는 `docs/134_actual_hwpx_manual_rehearsal_no_copy_hold.md`와 `docs/150_manual_preview_resume_gate.md`를 우선합니다. 비식별 작업 복사본 준비가 명시되기 전에는 실제 양식 수동 preview를 재개하지 않습니다.
