...
- HWPX 일원화를 유지하되 표 내부 데이터 자동화는 향후 Excel/한셀 연동 후보로 분리함
- 비식별 HWPX 작업 복사본이 없으면 실제 양식 수동 preview를 임의로 기록하지 않고 보류함
...
| 안내 문서 | `docs/00_chatgpt_handoff.md`, `docs/90_project_reorganization_after_superpowers.md`, `templates/hwpx/README.md`, `normalizers/README.md` | 오래된 현재상태 문구만 갱신 |
...
| 문서 템플릿 안정화 | 76~82% | Phase 4 진입 판단, manifestㆍplaceholder 정합성, style profile 확인 기준, 수동 preview gap log 기준, 실제 양식 후보 수동 절차, Git 제외 반복 검증 기준, 통합 점검, 사용자 확인 패킷, 준비 확인 기록, 표 틀 검수 범위 결정, 복사본 없음 보류 기록, 표 데이터 역할 분리 완료 |
| 실제 기관 양식 전환 | 42~52% | 저장소 밖 절차, 보류 조건, Git 제외 반복 검증 기준, 조건부 수동 리허설 진입 판단, 사용자 준비 확인, 표 틀 검수 범위 결정은 완료했으나 비식별 작업 복사본이 없어 실제 preview 결과 기록 전 |
| 외부 연동/API/Email 자동화 | 0~10% | 의도적으로 구현 보류 |

전체 시스템 구축 관점에서는 약 62~66% 진행으로 봅니다.

다만 "안전한 로컬 HWPX 보고서 초안 생성 MVP" 기준으로는 약 77~83%까지 진행된 상태입니다. 반대로 실제 운영 자동화와 외부 연동까지 포함한 최종 시스템 기준으로는 아직 절반을 조금 넘은 수준입니다.
...
10. HWPX 일원화 유지와 표 데이터 Excel/한셀 연동 후보 분리 결정 완료
11. 비식별 HWPX 작업 복사본 없음에 따른 실제 양식 수동 리허설 보류 기록 완료
12. HWPX 보고서와 Excel/한셀 표 데이터 역할 분리 범위 정리 완료
13. HWPX 보고서 4종 사용자 입력 템플릿의 표 데이터 후보 표시 기준 정리
14. 실제 기관 양식 투입은 사용자와 별도 승인 후 저장소 밖에서만 수행
15. API, Make.com, Email 자동화는 조직 보안 검토와 별도 승인 전까지 계속 보류
...
실제 양식 수동 리허설 전 사용자 준비 확인 기록은 `docs/132_actual_hwpx_manual_rehearsal_readiness_confirmation.md`에 반영했습니다.

HWPX 일원화 유지와 표 데이터 Excel/한셀 연동 후보 분리 결정은 `docs/133_hwpx_only_table_frame_decision.md`에 반영했습니다.

비식별 HWPX 작업 복사본 없음에 따른 수동 리허설 보류 기록은 `docs/134_actual_hwpx_manual_rehearsal_no_copy_hold.md`에 반영했습니다.

HWPX 보고서와 Excel/한셀 표 데이터 역할 분리 범위는 `docs/135_hwp_report_and_hancell_table_data_scope.md`에 반영했습니다.

후속 작업에서 HWPX 보고서 4종 사용자 입력 템플릿의 표 데이터 후보 표시 기준은 `docs/136_table_data_candidate_user_input_display_criteria.md`에 정리했습니다.

수동 preview 재개 조건은 `docs/150_manual_preview_resume_gate.md`에 정리했습니다. 현재 기준에서는 과거 준비 확인 기록보다 후속 복사본 없음 보류 기록과 재개 게이트를 우선합니다.
