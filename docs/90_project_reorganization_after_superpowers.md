...
최신 프로젝트 방향 재확인과 구형 문서 업데이트 기준은 `docs/124_project_direction_and_legacy_update_review.md`를 따르며, HWPX template manifest와 공통 placeholder 정합성 점검 결과는 `docs/125_hwpx_template_manifest_placeholder_consistency_review.md`를 따릅니다.
...
현재 최신 다음 작업은 `tasks/NEXT_STEP.md` 기준으로 style profile의 `[확인 필요]` 값 유지 기준과 기관 표준 서식값 수집 체크리스트를 문서로 정리하는 것입니다.
...
현재 후속 작업은 운영 자동화가 아니라 style profile의 `[확인 필요]` 값 유지 기준과 기관 표준 서식값 수집 체크리스트를 문서로 정리하는 것입니다.
