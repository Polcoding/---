...
| 한컴 열람 | 파일이 열리는지 | 실제 HWPX preview가 필요한 단계 |
| 글자 겹침 | 본문, 표, 번호체계의 겹침 여부 | 실제 HWPX 파일 열람 시 |
| 문단 배치 | 줄바꿈, 여백, 행 높이 | 실제 HWPX 파일 열람 시 |
| 표 틀 | 표 폭, 표 위치, 표 안 여백 | 표가 있는 HWPX 파일 열람 시 |
